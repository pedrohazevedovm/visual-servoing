...
def apply_boruvka_to_tensor(image_tensor, n_supix=200):
    """
    Aplica Boruvka Superpixel e reconstrói a imagem usando média por superpixel.

    image_tensor: (C, H, W) em [0,1]
    """
    # Tensor → numpy (H, W, C), converte de float [0, 1] para uint8 [0, 255]
    img_np = (image_tensor.permute(1, 2, 0).cpu().numpy() * 255.0)
    img_np = np.ascontiguousarray(img_np.astype(np.uint8))

    img_edge = np.zeros((img_np.shape[:2]), dtype=np.uint8)
    bosupix = boruvka_superpixel.BoruvkaSuperpixel()
    
    # Extrai features e constrói o grafo
    bosupix.build_2d(img_np, img_edge)
    
    # Calcula a cor média por superpixel. 3 é o número de canais da imagem (RGB)
    out = bosupix.average(n_supix, 3, img_np)

    # numpy → tensor, de volta para float [0, 1]
...
try:
    ...
    for title, n_supix in scenarios:
        ...
        # Se n_supix for None, usa a imagem original.
        # Se tiver n_supix, aplica o filtro.
        if n_supix is None:
