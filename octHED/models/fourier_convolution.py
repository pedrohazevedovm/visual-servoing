...
class FFCSE_block(nn.Module):
    def __init__(self, channels, ratio_g):
        ...
        self.conv1 = nn.Conv2d(
            channels, channels // r, kernel_size=1, bias=True
        )
    ...
    def forward(self, x):
        ...
        x_l = (
            0
            if self.conv_a2l is None
            else id_l * self.sigmoid(self.conv_a2l(x))
        )
        x_g = (
            0
            if self.conv_a2g is None
            else id_g * self.sigmoid(self.conv_a2g(x))
        )
        return x_l, x_g
...
class SpectralTransform(nn.Module):
    def __init__(
        self, in_channels, out_channels, stride=1, groups=1, enable_lfu=True
    ):
        # bn_layer not used
    ...
    def forward(self, x):
        ...
        if self.enable_lfu:
            ...
            xs = torch.cat(
                torch.split(xs, split_s_w, dim=-1), dim=1
            ).contiguous()
            xs = self.lfu(xs)
...
if __name__ == '__main__':
    a = FFC(15, 15, 3, 0.5, 0.5, enable_lfu=False)
